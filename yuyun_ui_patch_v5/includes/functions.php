...
// Safe wrapper to call setSetting and return success/exception
function setSettingSafe(string $key, ?string $value): bool {
    try {
        setSetting($key, $value);
        return true;
    } catch (Throwable $e) {
        error_log('setSettingSafe failed: ' . $e->getMessage());
        return false;
    }
}
...
function redirect(string $url): void {
    ...
    // fallback for headers already sent / safe redirect
    while (ob_get_level() > 0) {
        @ob_end_clean();
    }
    echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="refresh" content="0;url=' . e($url) . '"><script>window.location.href=' . json_encode($url, JSON_UNESCAPED_UNICODE) . ';</script></head><body></body></html>';
    exit;
}
