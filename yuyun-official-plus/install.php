...
function seedDefaults($db) {
    ...
    // 默认客户评价
    $testimonials = [
        ['content' => '语云科技的云服务器稳定性非常出色，帮助我们顺利度过了多次业务高峰。', 'author' => '某金融科技公司 CTO', 'company' => '金融科技', 'stars' => 5, 'sort_order' => 1, 'is_active' => 1],
        ['content' => '技术支持响应很快，全球化节点布局让我们的海外业务访问速度大幅提升。', 'author' => '某跨境电商平台 运维总监', 'company' => '跨境电商', 'stars' => 5, 'sort_order' => 2, 'is_active' => 1],
        ['content' => '从咨询到部署全流程专业高效，是企业数字化转型的可靠合作伙伴。', 'author' => '某制造业集团 信息中心主任', 'company' => '智能制造', 'stars' => 5, 'sort_order' => 3, 'is_active' => 1],
    ];
    foreach ($testimonials as $t) {
        if ($db->getType() === 'json') {
            $db->jsonInsert('testimonials', $t);
        } else {
            $db->execute("INSERT INTO testimonials (content, author, company, stars, sort_order, is_active) VALUES (?,?,?,?,?,?)", array_values($t));
        }
    }
}
?>
