...
if ($action === 'save') {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        ...
        if (empty($error) && $type === 'video' && empty($video_url)) {
            $error = '请输入视频链接地址';
        }
        ...
        if (empty($error) && empty($image) && $id > 0) {
            ...
            } elseif ($existing && empty($existing['image']) && $type === 'image') {
                $error = '请上传轮播图片';
            }
        }

        // 添加模式下图片类型必须上传图片
        if (empty($error) && empty($image) && $id <= 0 && $type === 'image') {
            $error = '请上传轮播图片';
        }
        ...
        if (empty($error)) {
            $data = [
                ...
                'type' => $type,
                'image' => $image,
                'video_url' => $type === 'video' ? $video_url : '',
                'link' => $link,
...
if ($action === 'list') {
    ...
    $slides = db_fetch_all(
        "SELECT * FROM carousel ORDER BY sort_order ASC, id DESC LIMIT ? OFFSET ?",
        [$per_page, $offset]
    );
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <h3>轮播图管理</h3>
            <a href="<?php echo admin_url('carousel.php?action=add'); ?>" class="btn btn-primary">
        ...
        <div class="table-container">
            <table class="data-table">
                ...
                <tbody>
                    <?php if (empty($slides)): ?>
                        <tr>
                            <td colspan="8" style="text-align:center;color:#999;padding:40px;">暂无轮播图数据</td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo admin_url('carousel.php?action=edit&id=' . $s['id']); ?>" title="<?php echo htmlspecialchars($s['title']); ?>">
                            ...
                            <td>
                                <span class="badge <?php echo $s['type'] === 'video' ? 'badge-success' : 'badge-info'; ?>">
                                    <?php echo $s['type'] === 'video' ? '视频' : '图片'; ?>
                                </span>
                            </td>
                            ...
                            <td class="table-actions">
                                <a href="<?php echo admin_url('carousel.php?action=edit&id=' . $s['id']); ?>" class="btn btn-sm btn-secondary" title="编辑">
                                    <i class="fas fa-edit"></i> 编辑
                                </a>
                                <form method="post" action="<?php echo admin_url('carousel.php?action=delete&id=' . $s['id']); ?>" style="display:inline;" onsubmit="return confirm('确定要删除轮播图「<?php echo htmlspecialchars(addslashes($s['title'])); ?>」吗？此操作不可恢复。');">
                                    <?php echo csrf_field(); ?>
        ...
        <?php echo pagination($total, $page, admin_url('carousel.php?'), $per_page); ?>
    </div>
...
if ($action === 'add' || $action === 'edit') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $slide['video_url'] = post('video_url', $slide['video_url']);
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <a href="<?php echo admin_url('carousel.php'); ?>" class="btn btn-secondary">
        ...
        <form method="post" action="<?php echo admin_url('carousel.php?action=save' . ($is_edit ? '&id=' . $slide['id'] : '')); ?>" enctype="multipart/form-data" class="card-form">
            <?php echo csrf_field(); ?>
            <?php if ($is_edit): ?>
            ...
            <div class="form-row">
                <div class="form-group form-group-main">
                    ...
                    <div class="radio-group">
                        ...
                        <label class="radio-label">
                            <input type="radio" name="type" value="video" <?php echo $slide['type'] === 'video' ? 'checked' : ''; ?>>
                            <span>视频</span>
                        </label>
                    </div>
                </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group form-group-side" id="video_url_group" style="<?php echo $slide['type'] !== 'video' ? 'display:none;' : ''; ?>">
                    <label for="video_url">视频链接 <span class="required">*</span></label>
                    <input type="text" id="video_url" name="video_url" value="<?php echo htmlspecialchars($slide['video_url']); ?>" placeholder="如：https://www.bilibili.com/video/xxx" maxlength="500">
                    <span class="form-hint">视频类型的跳转链接</span>
                </div>
            </div>
            ...
            <div class="form-actions">
                ...
                <a href="<?php echo admin_url('carousel.php'); ?>" class="btn btn-secondary">取消</a>
            </div>
        </form>
    </div>
    ...
    (function(){
        ...
        var videoGroup = document.getElementById('video_url_group');
