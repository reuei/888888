<div class="breadcrumb">订单管理 / 投诉管理</div>
<div class="page-header">
    <h2>投诉管理</h2>
    <a href="<?php echo url('admin/order'); ?>" class="btn btn-outline">订单列表</a>
</div>

<div class="card">
    <form class="search-bar" method="get" action="<?php echo url('admin/order/complaint'); ?>">
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <a href="<?php echo url('admin/order/detail') . '?id=' . $item['order_id']; ?>" class="btn btn-sm">订单详情</a>
                <?php if ($item['status'] == 0): ?>
                <a href="javascript:;" class="btn btn-sm btn-success" onclick="handleComplaint(<?php echo $item['id']; ?>)">处理</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; ?>
    ...
    <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 16px;">
        <?php if ($page > 1): ?>
        <a href="<?php echo url('admin/order/complaint') . '?page=' . ($page - 1) . '&status=' . $status; ?>" class="btn btn-sm btn-outline">上一页</a>
        <?php endif; ?>
        <span style="padding: 5px 10px; color: #64748B; font-size: 13px;"><?php echo $page; ?> / <?php echo $totalPages; ?> 共 <?php echo $total; ?> 条</span>
        <?php if ($page < $totalPages): ?>
        <a href="<?php echo url('admin/order/complaint') . '?page=' . ($page + 1) . '&status=' . $status; ?>" class="btn btn-sm btn-outline">下一页</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
async function handleComplaint(id) {
    ...
    const res = await fetch('<?php echo url('admin/order/complaintHandle'); ?>', { method: 'POST', body: form });
