<div class="breadcrumb">商户管理 / 商户列表</div>
<div class="page-header">
    <h2>商户列表</h2>
    <div>
        <a href="<?php echo url('admin/merchant/audit'); ?>" class="btn btn-outline">入驻审核</a>
        <a href="<?php echo url('admin/merchant/invite'); ?>" class="btn" style="margin-left: 8px;">邀请码</a>
    </div>
</div>

<div class="card">
    <form class="search-bar" method="get" action="<?php echo url('admin/merchant'); ?>">
    ...
    <table>
        ...
        <?php if (empty($list)): ?>
        <tr><td colspan="8" style="text-align: center; color: #64748B; padding: 40px;">暂无商户数据</td></tr>
        <?php else: ?>
    ...
    <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 16px;">
        <?php if ($page > 1): ?>
        <a href="<?php echo url('admin/merchant') . '?page=' . ($page - 1) . '&keyword=' . urlencode($keyword) . '&status=' . $status . '&subsite_id=' . $subsiteId; ?>" class="btn btn-sm btn-outline">上一页</a>
        <?php endif; ?>
        <span style="padding: 5px 10px; color: #64748B; font-size: 13px;"><?php echo $page; ?> / <?php echo $totalPages; ?> 共 <?php echo $total; ?> 条</span>
        <?php if ($page < $totalPages): ?>
        <a href="<?php echo url('admin/merchant') . '?page=' . ($page + 1) . '&keyword=' . urlencode($keyword) . '&status=' . $status . '&subsite_id=' . $subsiteId; ?>" class="btn btn-sm btn-outline">下一页</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
async function setStatus(id, status) {
    ...
    const res = await fetch('<?php echo url('admin/merchant/toggleStatus'); ?>', { method: 'POST', body: form });
...
async function forceOffline(id) {
    ...
    const form = new FormData();
    form.append('id', id);
    const res = await fetch('<?php echo url('admin/merchant/forceOffline'); ?>', { method: 'POST', body: form });
    const data = await res.json();
    alert(data.msg);
    if (data.code === 0) location.reload();
}
...
async function freezeFunds(id, action) {
    ...
    const res = await fetch('<?php echo url('admin/merchant/freezeFunds'); ?>', { method: 'POST', body: form });
    const data = await res.json();
    alert(data.msg);
    if (data.code === 0) location.reload();
}
