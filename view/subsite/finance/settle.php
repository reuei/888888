<div class="breadcrumb">财务结算 / 分站结算管理</div>
<div class="page-header">
    <h2>分站结算管理</h2>
    <a href="<?php echo url('subsite/finance/flow'); ?>" class="btn btn-outline">资金流水</a>
</div>

<div class="card" style="margin-bottom: 16px;">
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 16px;">
        <div style="text-align: center; padding: 12px; background: #F8FAFC; border-radius: 6px;">
            <div style="color: #64748B; font-size: 12px; margin-bottom: 4px;">结算笔数</div>
            <div style="font-size: 20px; font-weight: 600; color: #1F2937;"><?php echo $stat['total_count'] ?? 0; ?></div>
        </div>
        <div style="text-align: center; padding: 12px; background: #F8FAFC; border-radius: 6px;">
            <div style="color: #64748B; font-size: 12px; margin-bottom: 4px;">结算金额</div>
            <div style="font-size: 20px; font-weight: 600; color: #2563EB;">¥ <?php echo number_format($stat['total_amount'] ?? 0, 2); ?></div>
        </div>
        <div style="text-align: center; padding: 12px; background: #F8FAFC; border-radius: 6px;">
            <div style="color: #64748B; font-size: 12px; margin-bottom: 4px;">实付金额</div>
            <div style="font-size: 20px; font-weight: 600; color: #10B981;">¥ <?php echo number_format($stat['total_real'] ?? 0, 2); ?></div>
        </div>
        <div style="text-align: center; padding: 12px; background: #F8FAFC; border-radius: 6px;">
            <div style="color: #64748B; font-size: 12px; margin-bottom: 4px;">待处理金额</div>
            <div style="font-size: 20px; font-weight: 600; color: #F59E0B;">¥ <?php echo number_format($stat['pending_amount'] ?? 0, 2); ?></div>
        </div>
    </div>
</div>

<div class="card">
    <form class="search-bar" method="get" action="<?php echo url('subsite/finance/settle'); ?>">
    ...
    <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 16px;">
        <?php if ($page > 1): ?>
        <a href="<?php echo url('subsite/finance/settle') . '?page=' . ($page - 1) . '&keyword=' . urlencode($keyword) . '&status=' . $status; ?>" class="btn btn-sm btn-outline">上一页</a>
        <?php endif; ?>
        <span style="padding: 5px 10px; color: #64748B; font-size: 13px;"><?php echo $page; ?> / <?php echo $totalPages; ?> 共 <?php echo $total; ?> 条</span>
        <?php if ($page < $totalPages): ?>
        <a href="<?php echo url('subsite/finance/settle') . '?page=' . ($page + 1) . '&keyword=' . urlencode($keyword) . '&status=' . $status; ?>" class="btn btn-sm btn-outline">下一页</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
async function updateSettle(id, status) {
    ...
    const res = await fetch('<?php echo url('subsite/finance/settleUpdate'); ?>', { method: 'POST', body: form });
