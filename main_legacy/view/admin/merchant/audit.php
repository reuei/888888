<div class="breadcrumb">商户管理 / 入驻审核</div>
...
<div class="card">
    <table>
        ...
        <?php if (empty($list)): ?>
        <tr><td colspan="8" style="text-align: center; color: #64748B; padding: 40px;">暂无待审核商户</td></tr>
        <?php else: ?>
    ...
    <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 16px;">
        <?php if ($page > 1): ?>
        <a href="<?php echo url('admin/merchant/audit') . '?page=' . ($page - 1); ?>" class="btn btn-sm btn-outline">上一页</a>
        <?php endif; ?>
        <span style="padding: 5px 10px; color: #64748B; font-size: 13px;"><?php echo $page; ?> / <?php echo $totalPages; ?> 共 <?php echo $total; ?> 条</span>
        <?php if ($page < $totalPages): ?>
        <a href="<?php echo url('admin/merchant/audit') . '?page=' . ($page + 1); ?>" class="btn btn-sm btn-outline">下一页</a>
        <?php endif; ?>
    </div>
    <?php endif; ?>
</div>
...
async function auditPass(id) {
    ...
    const res = await fetch('<?php echo url('admin/merchant/auditPass'); ?>', { method: 'POST', body: form });
...
async function auditReject(id) {
    ...
    form.append('id', id);
    form.append('remark', remark);
    const res = await fetch('<?php echo url('admin/merchant/auditReject'); ?>', { method: 'POST', body: form });
    const data = await res.json();
    alert(data.msg);
    if (data.code === 0) location.reload();
}
...
async function addRemark(id) {
    ...
    form.append('id', id);
    form.append('remark', remark);
    const res = await fetch('<?php echo url('admin/merchant/auditRemark'); ?>', { method: 'POST', body: form });
    const data = await res.json();
    alert(data.msg);
}
</script>
